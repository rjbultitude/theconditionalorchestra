 ...
 * @param  {Array} origArray        [origArray to duplicate]
 ...
 */
module.exports = function addMissingArrayItems(origArray, difference, amountToAdd, repeat) {
  //error check
  if (typeof difference !== 'number') {
    console.error('difference is not a number');
  }
  var _index = 0;
  var _newArr = origArray.map(function(item) {
    return item;
  });
  var _finalArr = [];
  var _diffArr = [];
  ...
  for (var i = 0; i < difference; i++) {
    ...
    } else if (_index === origArray.length - 1) {
      _index = 0;
