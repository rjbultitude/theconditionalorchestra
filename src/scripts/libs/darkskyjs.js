...
( function ( root, factory ) {
    ...
    */
     ...
     * @return boolean
     */
    DarkSky.prototype.getCurrentConditions = function getCurrentConditions( locations, appFn ) {
        ...
        Promise.all( allLocDataArr ).then( function ( forecasts ) {
            if ( forecasts.length === 0 || forecasts[ 0 ] === '' || forecasts[ 0 ] === null || forecasts[ 0 ] === undefined ) {
                ...
                for ( var i = 0, length = forecasts.length; i < length; i++ ) {
                    ...
                    currDataSets.push( new DarkSkyConditions( currJsonData.currently, locations[ i ].name ) );
                }
     ...
     * @return boolean
     */
    DarkSky.prototype.getForecastToday = function getForecastToday( locations, appFn ) {
        var locationsArr = checkObject( locations );
        var allLocDataArr = this.requestAllLocData( locationsArr );
        Promise.all( allLocDataArr ).then( function ( forecasts ) {
            if ( forecasts.length === 0 || forecasts[ 0 ] === '' || forecasts[ 0 ] === null || forecasts[ 0 ] === undefined ) {
                ...
                for ( var i = 0, length = forecasts.length; i < length; i++ ) {
                    ...
                    for ( var j = 0, dLength = dailyJsonData.hourly.data.length; j < dLength; j++ ) {
     ...
     * @return boolean
     */
    DarkSky.prototype.getForecastWeek = function getForecastWeek( locations, appFn ) {
        var locationsArr = checkObject( locations );
        var allLocDataArr = this.requestAllLocData( locationsArr );
        Promise.all( allLocDataArr ).then( function ( forecasts ) {
            if ( forecasts.length === 0 || forecasts[ 0 ] === '' || forecasts[ 0 ] === null || forecasts[ 0 ] === undefined ) {
